    ...
    return (
        <div className="border border-[#FFE2DE] group rounded-xl p-6">
            ...
            >
                <div >
                    <p
                        onMouseEnter={() => setShowModal(true)}>({reviews?.length})</p>
                </div>
                <div
                    onMouseEnter={() => setShowModal(true)} className='flex'>
                ...
                {showModal && (
                    <div
                        className="fixed inset-0 z-20 overflow-hidden flex items-center justify-center bg-black bg-opacity-80"
                        aria-labelledby="modal-title"
                        role="dialog"
                        aria-modal="true"
                    >
                        <button
                            className="absolute top-6 right text-gray-400 hover:text-gray-200"
                            onClick={() => setShowModal(false)}
                        >
                            <FaTimes size={24} />
                        </button>
                        <div className='mt-12 relative p-6 bg-gray-900 rounded-lg shadow-xl max-h-[90vh] w-full sm:max-w-4xl sm:w-full overflow-auto flex'>
